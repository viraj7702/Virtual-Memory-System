...
char *heap;
unsigned long numPhysPages; //number of pages in (1GB) physical memory
int physBitMapChars; //number of chars needed for physical memory bitmap (1 bit per page)
char *pbitmap; //physcial bitmap
unsigned long numVirtPages; //number of pages in (4GB) virt addr space
int virtBitMapChars; //number of chars needed for virt addr bitmap (1 bit per page)
char *vbitmap; //virtual bitmap
unsigned long **PD; //Outer-level page directory for the entire Page Table
int numOffsetBits; //number of bits for the page offset - offset into the page
int numPDbits; //number of bits for indexing PD
...
void free_bit_at_index(char *bitmap, int index) {
	int block = index/8;
	int offset = index%8;
	if(offset == 0)
...
//	memset(tlbitmap, 0, tlbitmapChars);
	...
	if((32-numOffsetBits) % 2 == 1)
		numPDbits = (32-numOffsetBits)/2 + 1;
	else
		numPDbits = (32-numOffsetBits)/2;
	numPTbits = (32-numOffsetBits)/2;

//	maxNumPDEs = pow(2, numPDbits);
//	maxNumPTEs = pow(2, numPTbits);
	
	PD = (unsigned long **) heap; //outer-level page directory takes up the first page of our "fake" physical memory (PD[0] ... PD[1023] are the PDEs of this page directory - for 4K pages) 
...
//look for a free spot in TLB, if none then evict something and put new translation in free TLB entry
	...
	unsigned long int mask = 1;
        ...
        mask-=1;
        unsigned long int offset = (unsigned long int)va & mask;
        unsigned long int VPN = (unsigned long int)va >> numOffsetBits;
	unsigned long int physAddr = (unsigned long int)pa - offset;
...
void *translate(pde_t **pgdir, void *va) { //changed return type from pte_t * to void *
	...
	if(check != NULL)
		return check;
	unsigned long int virtaddr = (unsigned long int)va; //cast va to unsigned long
	unsigned long int VPN = virtaddr >> numOffsetBits;
	unsigned long int PDindex = VPN >> numPTbits;
	unsigned long int x = 1;
	for(int i = 0; i < numPTbits; i++) {
		x*=2;
	}
	x-=1;
	unsigned long int PTindex = VPN & x;
	if(PD[PDindex] == NULL)
		return NULL;
	//will we ever try to translate addresses from pages that we previously freed?
	if(PD[PDindex][PTindex] == 0)
		return NULL;
	unsigned long int physAddr = PD[PDindex][PTindex];
...
page_map(pde_t **pgdir, void *va, void *pa)
{
	...
	unsigned long int virtaddr = (unsigned long int)va; //cast va to unsigned long
        ...
        unsigned long int PTindex = VPN & x;
	if(PD[PDindex] == NULL) {
		PD[PDindex] = (unsigned long int *)next_avail_phys_page();// memcpy(PD[PDindex], nextavailphyspage(), PGSIZE)
		if(PD[PDindex] == NULL) //error check to see if there are not enough physical pages in phys. memory to allocate space for inner level page table
			return 0;
	}
	PD[PDindex][PTindex] = (unsigned long int)(pa);//(unsigned long int)next_avail_phys_page();
	if(PD[PDindex][PTindex] == 0)
		return 0;
	tlbChecks++;
	tlbMisses++;
...
void *t_malloc(unsigned int num_bytes) {
	...
	if(num_bytes % PGSIZE != 0)
		numPages++;
	unsigned long int physAddr;
	unsigned long int virtAddr = (unsigned long int)(get_next_avail(numPages));
	unsigned long int mapVirtAddr = virtAddr;
	for(int i = 0; i < numPages; i++) {
		physAddr = (unsigned long int)next_avail_phys_page();
		if(physAddr == 0)
...
void t_free(void *va, int size) {
	...
	if(size % PGSIZE != 0)
		numPages++;
	unsigned long int physAddr;
	unsigned long int virtAddr = (unsigned long int)va;
	for(int i = 0; i < numPages; i++) {
		void *temp = translate(PD, (void *)virtAddr);
		if(temp == NULL)
			return;
		physAddr = (unsigned long int)temp;
		physAddr-=(unsigned long int)heap;
		unsigned long int PFN = physAddr >> numOffsetBits;
...
void put_value(void *va, void *val, int size) {
	...
	x-=1;
	unsigned long int offset = virtAddr & x;
	int fsize;
	pthread_mutex_lock(&mutex);
